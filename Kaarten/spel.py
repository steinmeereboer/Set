...
<<<<<<< HEAD
    ...
    punten_computer = 0
    set_stapel = set.maak_stapel()  #stapel met kaarten
    tafel = set.kaarten_op_tafel(set_stapel) #kaarten die op de tafel liggen
    ...
    while len(set_stapel) >= 0 and len(tafel)==12: #
        ...
        if geldigheid_invoer == True:
            set_klopt =  (set.set_kaart.derde_kaart(tafel[speler_invoer[0]],
...
=======
    ...
    while len(set_stapel) > 2:
        ...
        if time.perf_counter() - start < 5:
            ...
            tafel[speler_invoer[1]]) == tafel[speler_invoer[2]]):
>>>>>>> 8a82e0e8fd1d0ef74a139bf798fd51847d2b6377
...
<<<<<<< HEAD
        ...
        if set.set_kaart.vind_alle_sets(tafel) == []: #er zijn geen sets op tafel
...
=======
        ...
        if klasse.set_kaart.vind_alle_sets(tafel) == []:
                indices = 0, 1, 2
                tafel = [i for j, i in enumerate(tafel) if j not in indices]
>>>>>>> 8a82e0e8fd1d0ef74a139bf798fd51847d2b6377
                for i in range(3):
...
<<<<<<< HEAD
            ...
            sets = set.set_kaart.vind_alle_sets(tafel) #computer pakt een set
=======
        elif (time.perf_counter() - start >= 5 or
              klasse.set_kaart.derde_kaart(tafel[speler_invoer[0]], tafel[a[1]]) != tafel[a[2]]):
            sets = klasse.set_kaart.vind_alle_sets(tafel)
>>>>>>> 8a82e0e8fd1d0ef74a139bf798fd51847d2b6377
